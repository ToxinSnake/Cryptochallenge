...
public class Cracker2 {
	...
	public static void main(String[] args) {
		...
		byte[][] fullMatrix = generateMatrix(pubKey, allVars);
		...
		byte[][] reduced = reducedRowEchelon(fullMatrix);
		System.out.println("Done!");
		...
		for(int i = 0; i < inserted.length; i++) {
			byte[] tempCipher = encrypt(pubKey, allVars, inserted[i]);
	...
	public static byte[] getChitext(String path) throws IOException {
		byte[] allTextRaw = Files.readAllBytes(Paths.get(path));
		String allText = new String(allTextRaw, "UTF-8");
		String Chitext;
		...
		byte[] chiAsArray = new byte[splits.length - 1];
		for(int i = 1, j = 0; i < splits.length; i++, j++) {
			chiAsArray[j] = (byte) Integer.parseInt(splits[i].trim());
		}
		return chiAsArray;
	}
	 ...
	 *         Chitexten
	 */
	public static byte[][] generateMatrix(ArrayList<Expression> pubKey, HashSet<String> allVars) {
		int varCount;
...
//				int varNum = iterator.charAt(2) - '0';
			...
			for (Expression iterator : pubKey) {
				...
				cipherText[expressionCounter] = (byte) (iterator.evaluate() % 2);
			...
			for (int j = 0; j < clearText.length; j++) {
				for (int s = 0; s < cipherText.length; s++) {
					fullMatrix[i][rowCounter] = (byte) (clearText[j] * cipherText[s]);
	...
	private static int getN(byte[][] matrix) {
		return matrix[0].length;
	}

	private static int getM(byte[][] matrix) {
	 ...
	 * @param rowNoB
	 */
	private static void swapRow(byte[][] matrix, int rowNoA, int rowNoB) {
		byte[] rowA = matrix[rowNoA];
		byte[] rowB = matrix[rowNoB];
		matrix[rowNoA] = rowB;
		matrix[rowNoB] = rowA;
	}
	 ...
	 * @param rowNoB
	 */
	private static void xorRow(byte[][] matrix, int rowNoA, int rowNoB) {
		byte[] rowA = matrix[rowNoA];
		for (int i = 0; i < matrix[0].length; i++) {
			matrix[rowNoB][i] = (byte) (matrix[rowNoB][i] ^ rowA[i]);
		}
	}
	 ...
	 */
	public static byte[][] specialSolutions(byte[][] matrix) {
		...
		HashSet<byte[]> specialSolutions = new HashSet<byte[]>();
		for (Integer openVarIndex : openVars) {
			// Kopie von erweiterter Zeilenstufenform anlegen
			byte[][] copy = new byte[getM(matrix)][];
			for (int i = 0; i < matrix.length; i++) {
				copy[i] = Arrays.copyOf(matrix[i], matrix[i].length);
			}
			setOtherVarsToZero(copy, openVarIndex);
			byte[] special = solveMatrix(copy, openVarIndex);
			specialSolutions.add(special);
		}
		
		byte[][] rtValue = new byte[specialSolutions.size()][numberOfVars*numberOfVars];
		int counter = 0;
		//Hashset in int Array übertragen
		for(byte[] solution : specialSolutions) {
		...
		if (debug) {
			...
			for (byte[] iterator : specialSolutions) {
				System.out.print("( ");
	 ...
	 */
	private static byte[] solveMatrix(byte[][] matrix, int openVar) {
		byte[] solution = new byte[numberOfVars * numberOfVars];
		int pivotIndex = 0;
		for (int i = 0; i < getM(matrix); i++) {
			int rowSolution = 0;
			...
			if (pivotIndex != -1) {
				...
				solution[pivotIndex] = (byte) ((rowSolution + 1) % 2);
			} else {
				break;
			}
		}
	 ...
	 */
	private static void setOtherVarsToZero(byte[][] matrix, int varIndex) {
		int pivotIndex = 0;
		for (int i = 0; i < getM(matrix); i++) {
			pivotIndex = getPivotIndex(matrix[i]);
	...
	private static int getPivotIndex(byte[] row) {
	...
	public static byte[][] insertChitext(byte[][] specials, byte[] chitext){
		byte[][] inserted = new byte[specials.length][numberOfVars];
		byte[] temp = new byte[numberOfVars*numberOfVars];
		...
		for(int sp = 0; sp < specials.length; sp++) {
			for(int outer = 0; outer < specials[sp].length;) {
				for(int inner = 0; inner < chitext.length; inner++, outer++) {
					temp[outer] = (byte) (specials[sp][outer] * chitext[inner]);
				}
			}
			...
			for(int outer = 0, insertCounter=0; outer < temp.length; insertCounter++) {
				...
				inserted[sp][insertCounter] = (byte) currentVar;
			}
		}
		return inserted;
	}
	
	public static byte[] encrypt(ArrayList<Expression> pubKey, HashSet<String> allVars, byte[] clearText) {
		byte[] cipherText = new byte[numberOfVars];
		HashMap<String, Double> values = new HashMap<String, Double>();
		for (String iterator : allVars) {
			int varNum = Integer.parseInt(iterator.substring(2));
			double varVal = clearText[varNum - 1];
